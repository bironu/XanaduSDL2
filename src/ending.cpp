...
static image_t kanji[KANJI_PAGE_ROW][KANJI_PAGE_COL];
static image_t *kanji_base;
static image_t *msg;
static int msg_y;
...
static int draw_kanji_text(image_t *img, int x, int y, int *code);
...
static int init(void)
{
  ...
  for (i = 0; i < KANJI_PAGE_ROW; i++) {
    for (j = 0; j < KANJI_PAGE_COL; j++) {
      subsection_image(kanji_base, j * KANJI_WIDTH, i * KANJI_HEIGHT,
                       KANJI_WIDTH, KANJI_HEIGHT, &kanji[i][j]);
    }
  }

  /* メッセージ用のオフスクリーンバッファ */
  msg = create_image(clip_endingroll->width,
                     clip_endingroll->height + KANJI_HEIGHT);
  if (!msg) {
    return 1;
  }
  msg->mask = kanji_base->mask;
  fill_image(msg, 0, 0, msg->width, msg->height, black_pixel);
...
void ending_enter(void)
{
  if (!kanji_base) {
    ...
    if (!in_scenario2()) {
      /* スクリーンのコピーを作成 */
      free(visual_image);
      visual_image = create_image(clip_overall->width, clip_overall->height);
      if (!visual_image) {
        restore_context();
      }
...
void ending_loop(void)
{
  ...
  if (msg_y > KANJI_HEIGHT) {
    ...
    if (kanji_code_top < kanji_code_end) {
      ...
      /* 最下行に描画 */
      n = draw_kanji_text(msg, 0, msg->height - KANJI_HEIGHT, kanji_code_top);
      kanji_code_top += n;
    } else {
      msg_rest_rows++;

      /* 最後の行がスクリーンから見えなくなった？ */
      if (msg_rest_rows > msg->height / KANJI_HEIGHT) {
        wait_forever();
      }
    }
  }
  subsection_image(visual_image, rect_endingroll.x, rect_endingroll.y,
...
int draw_kanji_text(image_t *img, int x, int y, int *code)
{
  int row, col;
  int n = 0;
  for (; *code >= 0; code++, n++) {
