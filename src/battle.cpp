...
/* モンスターステータス関連 */
static image_t *battle_monster_image;
...
/* 部屋の四辺のドアの位置 */
const point_t room_door_position[4] = {
  { 4, 8 }, { 0, 4 }, { 8, 4 }, { 4, 0 }
};

const int battle_frame_user[10] = { 8, 0, 8, 2, 0, 6, 2, 0, 6, 2 };
...
void battle_enter(void)
{
  ...
  if (in_battle()) {
    ...
    status_draw_text(7, 0, monster_status->name, red_pixel);
    
    for (i = 0; i < battle_max_monsters; i++) {
      int row = i + STATUS_MONSTER_HP_LINE;
      if (battle_monsters[i].HP >= 0)
        status_draw_integer(row, 5, battle_monsters[i].HP, white_pixel);
      else
        status_draw_text(row, 5, "Dead !!", red_pixel);
    }
    for (; i < MAX_MEMBER; i++) {
